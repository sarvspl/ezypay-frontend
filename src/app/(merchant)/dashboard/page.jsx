...
function CredentialsAndApk({ merchant }) {
  const { refreshMerchant } = useMerchant();
  const [unlockOpen, setUnlockOpen] = useState(false);
  const locked = !merchant.keys_unlocked;
      ...
      <div className="lg:col-span-2 card p-5 sm:p-6">
        ...
        {locked ? (
          <div className="mt-4 rounded-lg border border-dashed border-slate-300 bg-slate-50 p-5 text-center">
            <div className="mx-auto w-10 h-10 rounded-full bg-slate-200 text-slate-500 flex items-center justify-center">
            ...
            <div className="font-semibold text-slate-900 mt-2">Your keys are locked</div>
            <p className="text-sm text-slate-500 mt-1 max-w-sm mx-auto">
              Unlock your API key & device auth key with a one-time payment of{' '}
              <strong>{formatMoney(Number(merchant.key_unlock_fee || 0), merchant.currency)}</strong> from your wallet.
            </p>
            <button onClick={() => setUnlockOpen(true)} className="btn-primary mt-4 !py-2">Unlock keys</button>
          </div>
        ) : (
          <div className="mt-4 space-y-4">
